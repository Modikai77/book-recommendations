...
export default async function BookDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!book) {
    ...
    return (
      <SiteShell>
        <section className="rounded-[1.75rem] border border-stone-200 bg-white/80 p-5 md:rounded-[2rem] md:p-6">
          <p className="text-xs uppercase tracking-[0.35em] text-stone-500">{fallback.author}</p>
          <h1 className="mt-2 font-serif text-3xl sm:text-4xl md:text-5xl">{fallback.title}</h1>
  ...
  return (
    <SiteShell>
      <section className="space-y-5 md:space-y-6">
        <div className="rounded-[1.75rem] border border-stone-200 bg-white/80 p-5 md:rounded-[2rem] md:p-6">
          <p className="text-xs uppercase tracking-[0.35em] text-stone-500">{book.author}</p>
          <h1 className="mt-2 font-serif text-3xl sm:text-4xl md:text-5xl">{book.title}</h1>
          <p className="mt-6 text-base leading-8 text-stone-700">
